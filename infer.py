...
def detect_image(image_path, model_path, class_list):
    ...
    model.eval()
    path = sorted(os.listdir(image_path))
    path = sorted(path,key=lambda x: int(os.path.splitext(x)[0]))
    print(path)
    for img_name in path:
        ...
        with torch.no_grad():
            ...
            for j in range(idxs[0].shape[0]):
                ...
                x1 = int(bbox[0] / scale) #xmin
                y1 = int(bbox[1] / scale) #ymin
                x2 = int(bbox[2] / scale) #xmax
                y2 = int(bbox[3] / scale) #ymax
                
                print('classification',int(classification[idxs[0][j]]))
            ...
            score_list.reverse()
            label_list.reverse()
            image_prediction = {'bbox': box, 'score': score_list ,'label': label_list}
...
if __name__ == '__main__':
    ...
    with open('309551053_13.json', 'w') as outfile:
